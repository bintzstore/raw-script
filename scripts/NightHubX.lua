...
-- │  STATE                          │
-- └─────────────────────────────────┘
local Prefix1, Prefix2 = {}, {}   -- DB1 (sumber 1: game asli)
local DB2P1, DB2P2    = {}, {}   -- DB2 (sumber 2: CSV github)
...
HDivR(268)
Lbl(rightCtrl,{
	Size=UDim2.new(1,-10,0,10),Position=UDim2.new(0,5,1,-24),
	Text="Dev: BintzOffc",Font=Enum.Font.GothamBold,TextSize=7,
	TextColor3=P.txt2,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=12,
})
Lbl(rightCtrl,{
	Size=UDim2.new(1,-10,0,10),Position=UDim2.new(0,5,1,-13),
	Text="TikTok: NightHubX",Font=Enum.Font.Gotham,TextSize=7,
	TextColor3=P.txt2,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=12,
})
...
local wordLabels = {}  -- Frame label, bukan tombol
local function MkWLbl(i)
	local row=Frame(scroll,{
		Name="R"..i, Size=UDim2.new(1,0,0,24), LayoutOrder=i,
	...
	Lbl(row,{
		Size=UDim2.new(0,14,1,0),Position=UDim2.new(0,3,0,0),
		Text=tostring(i),Font=Enum.Font.GothamBold,TextSize=7,
		TextColor3=P.txt2,ZIndex=14,
	})

	Lbl(row,{
		Name="W",Size=UDim2.new(1,-18,1,0),Position=UDim2.new(0,18,0,0),
		Text="",Font=Enum.Font.GothamMedium,TextSize=11,
		TextColor3=P.txt0,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=14,
	})

	-- dot warna bahaya di kanan (tanpa teks)
	local dot=Frame(row,{
		Name="D",Size=UDim2.new(0,6,0,6),Position=UDim2.new(1,-10,0.5,-3),
		BackgroundColor3=P.ok,ZIndex=15,
	})
	RC(dot,3)

	table.insert(wordLabels,row)
end
for i=1,MAX_BTN do MkWLbl(i) end
...
local function FindOpts(prefix)
	...
	local function SearchIn(p1tbl, p2tbl)
		local list = #prefix>=2 and p2tbl[string.sub(prefix,1,2)] or p1tbl[string.sub(prefix,1,1)]
	...
	-- Cari di DB1 dulu
	local f = SearchIn(Prefix1, Prefix2)

	-- Kalau DB1 kosong, fallback ke DB2
	if #f == 0 then
		f = SearchIn(DB2P1, DB2P2)
	end
...
UpdatePreview=function()
	...
	if not Options or #Options==0 then
		...
		for _,r in ipairs(wordLabels) do r.Visible=false end
		return
	end
	...
	for i,row in ipairs(wordLabels) do
		if Options[i] then
			local ww=Options[i]
			local wL=row:FindFirstChild("W"); if wL then wL.Text="  "..ww end
			local dot=row:FindFirstChild("D")
			if dot then
				local _,dcol=Danger(ww); dot.BackgroundColor3=dcol
			end
			row.Visible=true
		else
			row.Visible=false
...
-- ─────────────────────────────────────────────
-- LOAD WORDLIST
-- DB1 = sumber 1 (game asli txt)
-- DB2 = sumber 2 (CSV github) — hanya kata yang belum ada di DB1
-- FindOpts cari di DB1 dulu, kalau kosong fallback ke DB2
-- ─────────────────────────────────────────────
task.spawn(function()
	statusLbl.Text="⏳ Memuat kamus..."

	local DB1Words = {}  -- set kata di DB1
	local DB2Words = {}  -- set kata di DB2

	local function IndexWord(w, p1tbl, p2tbl, setTbl)
		w = string.lower(w):gsub("%s","")
		w = w:match("^([a-z]+)") or ""
		if #w < 3 then return end
		if setTbl[w] then return end
		setTbl[w] = true
		local p1 = string.sub(w,1,1)
		local p2 = string.sub(w,1,2)
		p1tbl[p1] = p1tbl[p1] or {}; table.insert(p1tbl[p1], w)
		p2tbl[p2] = p2tbl[p2] or {}; table.insert(p2tbl[p2], w)
	end

	-- ── DB1: txt (sumber game asli) ──
	statusLbl.Text="⏳ DB1: kbbi.txt..."
	...
	if ok1 and res1 then
		for line in string.gmatch(res1,"[^\r\n]+") do
			IndexWord(line, Prefix1, Prefix2, DB1Words)
		end
	end

	-- ── DB2: CSV github — skip kata yang sudah ada di DB1 ──
	statusLbl.Text="⏳ DB2: kbbi.csv..."
	local ok2,res2 = pcall(function()
		return game:HttpGet("https://raw.githubusercontent.com/Hidayathamir/kata-kbbi-github/refs/heads/main/kbbi.csv")
	end)
	if ok2 and res2 then
		local isFirst = true
		for line in string.gmatch(res2,"[^\r\n]+") do
			if isFirst then
				isFirst = false  -- skip header CSV
			else
				local word = line:match("^([^,]+)")
				if word then
					local w = string.lower(word):gsub("%s","")
					w = w:match("^([a-z]+)") or ""
					-- hanya masuk DB2 kalau TIDAK ada di DB1
					if #w >= 3 and not DB1Words[w] then
						IndexWord(w, DB2P1, DB2P2, DB2Words)
					end
				end
			end
		end
	...
	local wc1=0; for _ in pairs(DB1Words) do wc1=wc1+1 end
	local wc2=0; for _ in pairs(DB2Words) do wc2=wc2+1 end
	local wc = wc1 + wc2

	for l = string.byte("a"), string.byte("z") do
		local c = string.char(l)
		local n1 = Prefix1[c] and #Prefix1[c] or 0
		local n2 = DB2P1[c]   and #DB2P1[c]   or 0
		LetterWordCount[c] = n1 + n2
	end

	Ready=true; AutoReady=true
	statusLbl.Text="✓ "..wc.." kata"; statusLbl.TextColor3=P.ok
	Toast("loaded","Siap!","DB1:"..wc1.." DB2:"..wc2.." · "..wc.." kata",3)
	print("[NightHubX v6] DB1="..wc1.." DB2="..wc2.." total="..wc)
end)
...
local function DoAuto(ts)
	...
	if AntiHardEnabled then
		-- Anti Hardword ON: hindari akhiran huruf susah
		local safe={}
		...
		-- Anti Hardword OFF: prioritaskan kata yang berakhiran huruf HARD
		-- supaya lawan kesulitan (strategi agresif)
		local hard={}
		for _,w in ipairs(Options) do
			if HARD_END[string.sub(w,-1)] then table.insert(hard,w) end
		end
		if #hard>0 then cands=hard end
	end
	...
	-- Prefix 3 huruf atau lebih: ketik tapi TIDAK langsung FireServer submit
	-- karena mungkin game masih menunggu user menyelesaikan kata
	-- Kita tetap ketik seluruh sisa kata, lalu submit normal
	local rem=string.sub(word,#CurrentLetter+1)

	-- Kalau prefix panjang (3+), beri jeda tambahan sebelum submit
	-- supaya game engine mendeteksi ketikan lebih natural
	local extraDelay = #CurrentLetter >= 3 and 0.15 or 0

	if not TypeWord(rem,ts) or not AutoEnabled then return end

	local sd=TYPING.subMin+math.random()*(TYPING.subMax-TYPING.subMin)+extraDelay
...
MatchUI.OnClientEvent:Connect(function(ev,data)
	...
	elseif ev=="Mistake" then
		AutoAnswered=false
		if AutoEnabled then
			autoStat.Text="🔄 Salah! Menghapus..."; autoStat.TextColor3=P.orange
			Toast("wrong","Salah","Menghapus...",1.5)
			-- Hapus teks yang salah santai kayak backspace manual
			task.spawn(function()
				-- Ambil panjang kata yang barusan diketik dari LastWord
				local wrongWord = LastWord or ""
				local remLen = wrongWord ~= "" and #string.sub(wrongWord, #CurrentLetter+1) or 0

				-- Hapus karakter per karakter dari kanan ke kiri
				local current = wrongWord ~= "" and string.sub(wrongWord, #CurrentLetter+1) or ""
				for i = #current, 1, -1 do
					if not ScriptActive then return end
					current = string.sub(current, 1, i-1)
					BillboardUpd:FireServer(current)
					-- delay hapus santai seperti orang backspace manual
					task.wait(math.random(8,18)/100)
				end
				-- Setelah hapus selesai, beri jeda lalu cari kata baru
				task.wait(0.3)
				UpdatePreview()
				task.wait(0.2)
				TriggerAuto()
