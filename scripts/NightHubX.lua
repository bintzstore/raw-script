...
-- ── Fake Type listener ──
-- Cara kerja:
--   1. User ketik kata asal di textbox (misal "kasssss", awalan harus match CurrentLetter)
--   2. Script monitor perubahan text, ambil huruf PERTAMA (validasi = CurrentLetter) dan TERAKHIR
--   3. Cari kata valid di kamus: berawalan CurrentLetter, berakhiran huruf terakhir ketikan palsu
--   4. Saat user submit (tekan Enter / FocusLost), intercept → FireServer kata NYATA
--      bukan kata yang diketik user
local function SetupFakeTypeListener()
	...
	-- Polling loop cari textbox game (karena mungkin spawn dynamic)
	task.spawn(function()
		local input = nil
		for _=1,20 do
		...
		-- State tracking
		local pendingRealWord = nil   -- kata nyata yang sudah siap disubmit
		local lastDisplayedWord = ""

		-- Monitor perubahan teks: update preview kata nyata secara realtime
		local textConn = input:GetPropertyChangedSignal("Text"):Connect(function()
			...
			if raw == lastDisplayedWord then return end
			lastDisplayedWord = raw

			-- Minimal 2 karakter baru proses
			if #raw < 2 then
				pendingRealWord = nil
				if autoAnswerStatusLabel then
					autoAnswerStatusLabel.Text = "🎭 Ketik kata palsu (min 2 huruf)..."
					autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(0,200,180)
				end
				return
			end

			-- Validasi: huruf pertama ketikan harus cocok dengan CurrentLetter
			local firstChar = string.sub(raw,1,1)
			if firstChar ~= string.sub(CurrentLetter,1,1) then
				pendingRealWord = nil
				if autoAnswerStatusLabel then
					autoAnswerStatusLabel.Text = '🎭 Huruf awal harus "'..CurrentLetter:upper()..'" (kamu ketik "'..firstChar:upper()..'")'
					autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(255,120,50)
				end
				return
			end

			-- Ambil huruf TERAKHIR ketikan palsu → cari kata nyata berakhiran itu
			local lastChar = string.sub(raw,-1)
			local realOptions = FindOptionsByEnding(CurrentLetter, lastChar)

			if not realOptions or #realOptions == 0 then
				pendingRealWord = nil
				if autoAnswerStatusLabel then
					autoAnswerStatusLabel.Text = '🎭 Tidak ada kata akhiran "'..lastChar:upper()..'" untuk "'..CurrentLetter:upper()..'"'
					autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(255,100,80)
				end
				return
			end

			pendingRealWord = realOptions[1]
			if autoAnswerStatusLabel then
				autoAnswerStatusLabel.Text = '🎭 "'..raw..'" → akan submit: "'..pendingRealWord..'" (→'..lastChar:upper()..')'
				autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(0,220,190)
			end
		end)

		-- Intercept FocusLost (user tekan Enter = submit)
		local focusConn = input.FocusLost:Connect(function(enterPressed)
			if not FakeTypeEnabled then return end
			if not enterPressed then return end          -- hanya kalau Enter, bukan klik luar
			...
			-- Submit kata NYATA lewat RemoteEvent
			...
			SendNotification("auto","🎭 Fake Type Submit!",
				'"'..wordToSend..'" → '..lc:upper().." "..dangerText.." | (ketikan palsu diintersep)", 3)
			if autoAnswerStatusLabel then
				autoAnswerStatusLabel.Text = '🎭 ✅ "'..wordToSend..'" terkirim! ('..lc:upper()..')'
				autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(0,240,200)
			end
			UpdatePreview()
		end)

		-- Simpan semua koneksi dalam satu wrapper agar bisa diputus sekaligus
		FakeTypeConnection = {
			Disconnect = function()
