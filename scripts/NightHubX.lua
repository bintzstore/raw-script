...
fakeTypeInfo.Text = "Ketik kata apa saja → huruf akhir kata itu\ndipakai cari kata asli lalu disubmit otomatis"
...
local function SetupFakeTypeListener()
	...
	local lastFakeText = ""
	local fakeSubmitDebounce = false

	FakeTypeConnection = input:GetPropertyChangedSignal("Text"):Connect(function()
		if not FakeTypeEnabled then return end
		if not Ready or not CurrentLetter then return end
		if fakeSubmitDebounce then return end

		local rawText = string.lower(input.Text):gsub("[^a-z]","")
		if rawText == lastFakeText then return end
		lastFakeText = rawText

		-- Hanya proses kalau teks cukup panjang (minimal 2 karakter ketikan)
		if #rawText < 2 then return end

		-- Ambil huruf akhir dari kata palsu yang diketik
		local fakeLastChar = string.sub(rawText, -1)

		-- Cari kata nyata yang berawalan CurrentLetter DAN berakhiran fakeLastChar
		local realOptions = FindOptionsByEnding(CurrentLetter, fakeLastChar)
		if not realOptions or #realOptions == 0 then
			if autoAnswerStatusLabel then
				autoAnswerStatusLabel.Text = '🎭 Tidak ada kata akhiran "'..fakeLastChar:upper()..'" untuk "'..CurrentLetter:upper()..'"'
				autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(255,150,50)
			end
			return
		end

		local realWord = realOptions[1]
		if autoAnswerStatusLabel then
			autoAnswerStatusLabel.Text = '🎭 Fake "'..rawText..'" → real: "'..realWord..'" (akhiran '..fakeLastChar:upper()..')'
			autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(0,210,180)
		end

		-- Debounce: tunggu 0.5 detik setelah user berhenti ketik baru submit
		fakeSubmitDebounce = true
		local snapText = rawText
		task.delay(0.5, function()
			-- Cek apakah text berubah lagi
			local currentRaw = string.lower(input.Text):gsub("[^a-z]","")
			if currentRaw ~= snapText then
				fakeSubmitDebounce = false
				return
			end
			if not FakeTypeEnabled then fakeSubmitDebounce=false; return end

			-- Submit kata nyata
			local remaining_real = string.sub(realWord, #CurrentLetter+1)
			MarkWordAsUsed(realWord, "fake_type")
			LastSubmittedWord = realWord
			SubmitWord:FireServer(remaining_real)

			local lc2 = string.sub(realWord,-1)
			local _,dangerText2 = GetDangerCategory(realWord)
			SendNotification("auto","🎭 Fake Type Submit",
				'Palsu: "'..rawText..'" → Nyata: "'..realWord..'" → '..lc2:upper().." "..dangerText2, 3)
			if autoAnswerStatusLabel then
				autoAnswerStatusLabel.Text = '🎭 ✅ Submitted: "'..realWord..'" (dari fake "'..rawText..'")'
...
fakeTypeBtn.MouseButton1Click:Connect(function()
	...
	if FakeTypeEnabled then
		SendNotification("auto", "🎭 Fake Type ON",
			"Ketik kata apa saja di input game. Huruf akhir kata itu dipakai cari & submit kata nyata.", 4.5)
...
-- AUTO ANSWER ENGINE
-- ============================================================
-- FIND OPTIONS BY ENDING LETTER (untuk Fake Type)
-- Cari kata di seluruh kamus yang diawali CurrentLetter DAN diakhiri targetEnding
-- ============================================================
local function FindOptionsByEnding(prefix, targetEnding)
	prefix = string.lower(prefix):gsub("[^a-z]","")
	targetEnding = string.lower(targetEnding):gsub("[^a-z]","")
	if #targetEnding == 0 then return {} end
	...
	for _, word in ipairs(list) do
		if string.sub(word,1,#prefix) == prefix then
			if string.sub(word,-1) == targetEnding then
				if not seen[word] and not MatchUsedWords[word] then
					seen[word] = true
	...
	-- Sort: prioritaskan kata pendek supaya lebih cepat disubmit
...
-- ============================================================
-- HUMAN MODE TYPE WORD
-- Delay 0.7s sebelum ketik, sengaja typo+backspace, skip killer (sudah difilter di DoAutoAnswer)
-- ============================================================
local function HumanModeTypeWord(word, startPrefix, turnStartTime)
	local remaining_word = string.sub(word, #startPrefix+1)

	local elapsed = tick()-turnStartTime
	local rem = TYPING_CONFIG.hardDeadline - elapsed
	if rem <= 1.5 then
		BillboardUpdate:FireServer(remaining_word)
		return true
	end

	-- Pre-type delay (~0.7s) — seperti orang "nimbang" kata dulu
	local preD = HUMAN_CONFIG.preTypeDelayMin + math.random()*(HUMAN_CONFIG.preTypeDelayMax - HUMAN_CONFIG.preTypeDelayMin)
	local tLeft2 = TYPING_CONFIG.hardDeadline-(tick()-turnStartTime)
	if tLeft2 < preD + 2.5 then preD = math.max(0.2, tLeft2-2.5) end
	task.wait(preD)
	if not AutoAnswerEnabled then return false end

	-- Estimasi waktu + speed multiplier
	local estTime = #remaining_word * (TYPING_CONFIG.charDelayMin+TYPING_CONFIG.charDelayMax)/2
	estTime = estTime + TYPING_CONFIG.submitDelayMax + 1.5 -- buffer untuk koreksi typo
	local rem2 = TYPING_CONFIG.hardDeadline-(tick()-turnStartTime)
	local speedMul = 1.0
	if estTime > rem2-0.5 then speedMul = math.max((rem2-0.5)/estTime, 0.35) end

	local typed = ""  -- string yang sudah dikirim ke BillboardUpdate
	local i = 1
	while i <= #remaining_word do
		...
		local realChar = string.sub(remaining_word, i, i)

		-- Coba typo?
		if math.random() < HUMAN_CONFIG.typoChance and i < #remaining_word then
			local wrongChar = GetTypoChar(realChar)
			-- Kirim dengan karakter salah
			typed = typed .. wrongChar
			BillboardUpdate:FireServer(typed)

			local holdT = HUMAN_CONFIG.typoHoldMin + math.random()*(HUMAN_CONFIG.typoHoldMax-HUMAN_CONFIG.typoHoldMin)
			task.wait(holdT * speedMul)
			if not AutoAnswerEnabled then return false end

			-- "Backspace" — hapus karakter salah
			typed = string.sub(typed, 1, #typed-1)
			BillboardUpdate:FireServer(typed)
			task.wait(0.08 * speedMul)
			if not AutoAnswerEnabled then return false end

			-- Ketik yang benar
			typed = typed .. realChar
			BillboardUpdate:FireServer(typed)
		else
			-- Ketik normal
			typed = typed .. realChar
		...
		local baseD = TYPING_CONFIG.charDelayMin + math.random()*(TYPING_CONFIG.charDelayMax-TYPING_CONFIG.charDelayMin)
		local delay = baseD * speedMul

		-- Pause/hesitation sesekali
		if math.random() < TYPING_CONFIG.pauseChance and i < #remaining_word then
			local pauseD = TYPING_CONFIG.pauseDelayMin + math.random()*(TYPING_CONFIG.pauseDelayMax-TYPING_CONFIG.pauseDelayMin)
			delay = delay + pauseD * speedMul
		end
...
-- ============================================================
-- HUMAN TYPE WORD — versi normal (tanpa human mode)
-- ============================================================
local function HumanTypeWord(word, startPrefix, turnStartTime)
	local remaining_word = string.sub(word, #startPrefix+1)
	local elapsed = tick()-turnStartTime
	local rem = TYPING_CONFIG.hardDeadline - elapsed
	if rem<=1.0 then BillboardUpdate:FireServer(remaining_word); return true end

	local charsToType = #remaining_word
	local estTime = charsToType*(TYPING_CONFIG.charDelayMin+TYPING_CONFIG.charDelayMax)/2 + TYPING_CONFIG.submitDelayMax
	local speedMul = 1.0
	...
	for i=1,#remaining_word do
		if not AutoAnswerEnabled then return false end
		local tLeft = TYPING_CONFIG.hardDeadline-(tick()-turnStartTime)
		if tLeft<=0.8 then BillboardUpdate:FireServer(remaining_word); return true end
		BillboardUpdate:FireServer(string.sub(remaining_word,1,i))
		local baseD = TYPING_CONFIG.charDelayMin + math.random()*(TYPING_CONFIG.charDelayMax-TYPING_CONFIG.charDelayMin)
		local delay = baseD*speedMul
		if math.random()<TYPING_CONFIG.pauseChance and i<#remaining_word then
			delay = delay + (TYPING_CONFIG.pauseDelayMin+math.random()*(TYPING_CONFIG.pauseDelayMax-TYPING_CONFIG.pauseDelayMin))*speedMul
		end
		task.wait(delay)
	end
	return true
...
local function DoAutoAnswer(turnStartTime)
	...
	if not Options or #Options==0 then Options=FindOptions(CurrentLetter) end

	-- ── HUMAN MODE: filter keluar kata yang akhirannya killer ──
	local candidateOptions = Options
	if HumanModeEnabled and HUMAN_CONFIG.skipKillerLetters then
		local filtered = {}
		for _, w in ipairs(Options) do
			local lc = string.sub(w,-1)
			if not HUMAN_CONFIG.killerLettersToSkip[lc] then
				table.insert(filtered, w)
			end
		end
		-- Pakai filtered jika ada, kalau kosong fallback ke semua opsi
		if #filtered > 0 then candidateOptions = filtered end
	end

	if not candidateOptions or #candidateOptions==0 then
		if autoAnswerStatusLabel then
		...
		SendNotification("warning","NightHubX: Tidak Ada Kata","Semua kata mungkin sudah terpakai di match ini.",3)
		return
	end

	local word = candidateOptions[1]
	MarkWordAsUsed(word,"auto_answer"); LastSubmittedWord=word

	if autoAnswerStatusLabel then
		local modeTag = HumanModeEnabled and " 👤" or " 🤖"
		autoAnswerStatusLabel.Text='🤖 Mendeteksi: "'..word..'"'..modeTag
		autoAnswerStatusLabel.TextColor3=Color3.fromRGB(140,100,255)
	end

	-- ── Think delay ──
	local thinkMin = HumanModeEnabled and (TYPING_CONFIG.thinkDelayMin + 0.5) or TYPING_CONFIG.thinkDelayMin
	local thinkMax = HumanModeEnabled and (TYPING_CONFIG.thinkDelayMax + 0.8) or TYPING_CONFIG.thinkDelayMax
	local thinkD = thinkMin + math.random()*(thinkMax - thinkMin)
	local tLeft = TYPING_CONFIG.hardDeadline-(tick()-turnStartTime)
	if tLeft < thinkD+2.0 then thinkD=math.max(0.3,tLeft-2.0) end
	task.wait(thinkD)
	...
	local success
	if HumanModeEnabled then
		success = HumanModeTypeWord(word, CurrentLetter, turnStartTime)
	else
		success = HumanTypeWord(word, CurrentLetter, turnStartTime)
	end
	if not success or not AutoAnswerEnabled then return end

	local subD = TYPING_CONFIG.submitDelayMin + math.random()*(TYPING_CONFIG.submitDelayMax-TYPING_CONFIG.submitDelayMin)
	local tLeftNow = TYPING_CONFIG.hardDeadline-(tick()-turnStartTime)
	if tLeftNow < subD+0.3 then subD=math.max(0.1,tLeftNow-0.3) end
	task.wait(subD)
	if not AutoAnswerEnabled then return end

	local remaining_word = string.sub(word, #CurrentLetter+1)
	SubmitWord:FireServer(remaining_word)

	local lc = string.sub(word,-1)
	local _,dangerText = GetDangerCategory(word)
	local totalTime = string.format("%.1f", tick()-turnStartTime)
	if autoAnswerStatusLabel then
		autoAnswerStatusLabel.Text='✅ "'..word..'" '..dangerText..' ('..totalTime..'s)'
	end
	SendNotification("auto","Auto Detect 🤖",'"'..word..'" → '..lc:upper().." "..dangerText.." ["..totalTime.."s]",2.5)
	UpdatePreview()
end
...
print("[NightHubX v5] Fitur: AutoDetect | HumanMode (typo+delay+skipKiller) | FakeType (palsu→nyata)")
