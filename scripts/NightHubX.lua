...
-- ============================================================
-- TYPING CUSTOMIZATION STATE
-- ============================================================
local TypoEnabled    = false   -- ketik salah lalu hapus + ketik benar
local CustomCharDelay = 0.15  -- delay per karakter (0.1 – 3.0 detik)
...
autoSection.Size = UDim2.new(1,-16,0,158); autoSection.Position = UDim2.new(0,8,0,168)
...
autoTitle2.BackgroundTransparency = 1; autoTitle2.Text = "🤖  AUTO ANSWER"
...
local function UpdateAutoToggleVisual()
	if AutoAnswerEnabled then
		...
		autoToggleLabel.Text = "ON — Auto Answer AKTIF!"
...
autoToggleBtn.MouseButton1Click:Connect(function()
	...
	if AutoAnswerEnabled then
		SendNotification("auto", "Auto Answer AKTIF 🤖", "NightHubX jawab otomatis. DupFilter ON.", 4)
		autoAnswerStatusLabel.Text = "⏳ Menunggu giliran..."
		autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(140,100,255)
	else
		SendNotification("info", "Auto Answer NONAKTIF", "Mode manual aktif.", 2.5)
		autoAnswerStatusLabel.Text = ""
	end
end)

-- ============================================================
-- DELAY SLIDER (0.1s – 3.0s per karakter)
-- ============================================================
local DELAY_MIN = 0.1
local DELAY_MAX = 3.0

local delayRow = Instance.new("Frame", autoSection)
delayRow.Size = UDim2.new(1,-20,0,38); delayRow.Position = UDim2.new(0,10,0,66)
delayRow.BackgroundColor3 = Color3.fromRGB(14,11,28); delayRow.BorderSizePixel = 0; delayRow.ZIndex = 5
Instance.new("UICorner", delayRow).CornerRadius = UDim.new(0,8)
local delayRowStroke = Instance.new("UIStroke", delayRow)
delayRowStroke.Color = Color3.fromRGB(50,35,90); delayRowStroke.Thickness = 1; delayRowStroke.Transparency = 0.5

local delayLabel = Instance.new("TextLabel", delayRow)
delayLabel.Size = UDim2.new(0,80,0,14); delayLabel.Position = UDim2.new(0,8,0,4)
delayLabel.BackgroundTransparency = 1; delayLabel.Text = "⏱ DELAY KETIK"
delayLabel.Font = Enum.Font.GothamBold; delayLabel.TextSize = 8
delayLabel.TextColor3 = Color3.fromRGB(140,120,200); delayLabel.TextXAlignment = Enum.TextXAlignment.Left; delayLabel.ZIndex = 6

local delayValLabel = Instance.new("TextLabel", delayRow)
delayValLabel.Size = UDim2.new(0,45,0,14); delayValLabel.Position = UDim2.new(1,-53,0,4)
delayValLabel.BackgroundTransparency = 1; delayValLabel.Text = "0.15s"
delayValLabel.Font = Enum.Font.GothamBold; delayValLabel.TextSize = 9
delayValLabel.TextColor3 = Color3.fromRGB(160,130,255); delayValLabel.TextXAlignment = Enum.TextXAlignment.Right; delayValLabel.ZIndex = 6

-- Track slider
local sliderTrack = Instance.new("Frame", delayRow)
sliderTrack.Size = UDim2.new(1,-16,0,8); sliderTrack.Position = UDim2.new(0,8,0,24)
sliderTrack.BackgroundColor3 = Color3.fromRGB(30,24,55); sliderTrack.BorderSizePixel = 0; sliderTrack.ZIndex = 6
Instance.new("UICorner", sliderTrack).CornerRadius = UDim.new(1,0)

local sliderFill = Instance.new("Frame", sliderTrack)
sliderFill.Size = UDim2.new(0.017,0,1,0)  -- 0.1s / 3.0s range → (0.15-0.1)/(3.0-0.1) ≈ 0.017
sliderFill.BackgroundColor3 = Color3.fromRGB(120,80,255); sliderFill.BorderSizePixel = 0; sliderFill.ZIndex = 7
Instance.new("UICorner", sliderFill).CornerRadius = UDim.new(1,0)

local sliderKnob = Instance.new("Frame", sliderTrack)
sliderKnob.Size = UDim2.new(0,14,0,14); sliderKnob.Position = UDim2.new(0.017,0,0.5,-7)
sliderKnob.BackgroundColor3 = Color3.fromRGB(200,180,255); sliderKnob.BorderSizePixel = 0; sliderKnob.ZIndex = 8
Instance.new("UICorner", sliderKnob).CornerRadius = UDim.new(1,0)
local sliderKnobStroke = Instance.new("UIStroke", sliderKnob)
sliderKnobStroke.Color = Color3.fromRGB(120,80,255); sliderKnobStroke.Thickness = 1.5

-- Fungsi update nilai dari posisi (0–1)
local function SetSliderValue(pct)
	pct = math.clamp(pct, 0, 1)
	CustomCharDelay = DELAY_MIN + pct * (DELAY_MAX - DELAY_MIN)
	CustomCharDelay = math.floor(CustomCharDelay * 100 + 0.5) / 100  -- round 2 desimal

	sliderFill.Size = UDim2.new(pct, 0, 1, 0)
	-- Knob: offset -7 supaya center di titik pct
	sliderKnob.Position = UDim2.new(pct, -7, 0.5, -7)

	delayValLabel.Text = string.format("%.2fs", CustomCharDelay)

	-- Warna fill berubah: hijau (lambat/aman) → merah (cepat)
	local r = math.floor(pct * 255)
	local g = math.floor((1 - pct) * 200)
	local col = Color3.fromRGB(80 + r, 80 + g, 255 - r)
	sliderFill.BackgroundColor3 = col
	sliderKnob.BackgroundColor3 = Color3.fromRGB(220, 210, 255)
	sliderKnobStroke.Color = col
end

-- Set nilai awal (0.15s)
do
	local initPct = (CustomCharDelay - DELAY_MIN) / (DELAY_MAX - DELAY_MIN)
	SetSliderValue(initPct)
end
...
-- ============================================================
-- TOGGLE TYPO (ketik salah → hapus → ketik benar)
-- ============================================================
local typoRow = Instance.new("TextButton", autoSection)
typoRow.Size = UDim2.new(1,-20,0,26); typoRow.Position = UDim2.new(0,10,0,110)
typoRow.BackgroundColor3 = Color3.fromRGB(14,11,28); typoRow.Text = ""
typoRow.BorderSizePixel = 0; typoRow.AutoButtonColor = false; typoRow.ZIndex = 5
Instance.new("UICorner", typoRow).CornerRadius = UDim.new(0,8)
local typoRowStroke = Instance.new("UIStroke", typoRow)
typoRowStroke.Color = Color3.fromRGB(50,35,90); typoRowStroke.Thickness = 1; typoRowStroke.Transparency = 0.5

local typoTrack = Instance.new("Frame", typoRow)
typoTrack.Size = UDim2.new(0,34,0,16); typoTrack.Position = UDim2.new(0,6,0.5,-8)
typoTrack.BackgroundColor3 = Color3.fromRGB(40,30,65); typoTrack.BorderSizePixel = 0; typoTrack.ZIndex = 6
Instance.new("UICorner", typoTrack).CornerRadius = UDim.new(1,0)

local typoKnob = Instance.new("Frame", typoTrack)
typoKnob.Size = UDim2.new(0,12,0,12); typoKnob.Position = UDim2.new(0,2,0.5,-6)
typoKnob.BackgroundColor3 = Color3.fromRGB(130,120,165); typoKnob.BorderSizePixel = 0; typoKnob.ZIndex = 7
Instance.new("UICorner", typoKnob).CornerRadius = UDim.new(1,0)

local typoLabel = Instance.new("TextLabel", typoRow)
typoLabel.Size = UDim2.new(1,-52,1,0); typoLabel.Position = UDim2.new(0,46,0,0)
typoLabel.BackgroundTransparency = 1; typoLabel.Text = "❌ Typo OFF — ketik langsung benar"
typoLabel.Font = Enum.Font.GothamBold; typoLabel.TextSize = 9
typoLabel.TextColor3 = Color3.fromRGB(110,100,150); typoLabel.TextXAlignment = Enum.TextXAlignment.Left; typoLabel.ZIndex = 6

local function UpdateTypoVisual()
	if TypoEnabled then
		Tween(typoTrack, {BackgroundColor3=Color3.fromRGB(200,140,0)}, 0.22)
		Tween(typoKnob,  {Position=UDim2.new(1,-14,0.5,-6), BackgroundColor3=Color3.fromRGB(255,255,255)}, 0.22, Enum.EasingStyle.Back)
		Tween(typoRowStroke, {Color=Color3.fromRGB(220,160,0)}, 0.18)
		typoLabel.Text = "⚡ Typo ON — salah → hapus → benar"
		typoLabel.TextColor3 = Color3.fromRGB(255,195,50)
	else
		Tween(typoTrack, {BackgroundColor3=Color3.fromRGB(40,30,65)}, 0.22)
		Tween(typoKnob,  {Position=UDim2.new(0,2,0.5,-6), BackgroundColor3=Color3.fromRGB(130,120,165)}, 0.22, Enum.EasingStyle.Back)
		Tween(typoRowStroke, {Color=Color3.fromRGB(50,35,90)}, 0.18)
		typoLabel.Text = "❌ Typo OFF — ketik langsung benar"
		typoLabel.TextColor3 = Color3.fromRGB(110,100,150)
	end
end

typoRow.MouseEnter:Connect(function() Tween(typoRow,{BackgroundColor3=Color3.fromRGB(20,16,40)},0.12) end)
typoRow.MouseLeave:Connect(function() Tween(typoRow,{BackgroundColor3=Color3.fromRGB(14,11,28)},0.12) end)
typoRow.MouseButton1Click:Connect(function()
	TypoEnabled = not TypoEnabled
	UpdateTypoVisual()
	if TypoEnabled then
		SendNotification("warning", "⚡ Typo ON", "Setiap karakter: ketik salah dulu, hapus, lalu ketik benar.", 3)
	else
		SendNotification("info", "Typo OFF", "Ketik langsung benar, tanpa simulasi salah.", 2.5)
	end
end)

-- Info tips
local divLine = Instance.new("Frame", leftPanel)
divLine.Size = UDim2.new(1,-24,0,1); divLine.Position = UDim2.new(0,12,0,334)
...
tipsHeader2.Size = UDim2.new(1,-16,0,16); tipsHeader2.Position = UDim2.new(0,10,0,340)
tipsHeader2.BackgroundTransparency = 1; tipsHeader2.Text = "💡  PANDUAN"
tipsHeader2.Font = Enum.Font.GothamBold; tipsHeader2.TextSize = 10
...
local tips = {
	...
	"🤖 Auto = ambil dari opsi teratas",
	"⏱ Atur delay ketik per karakter",
	"⚡ Typo: salah → hapus → benar",
	"🚫 Kata terpakai hilang otomatis",
	"◆ Ganti tema | 🧠 Sort | ⏏ Unload",
}
for i, tip in ipairs(tips) do
	local tl = Instance.new("TextLabel", leftPanel)
	tl.Size = UDim2.new(1,-22,0,14); tl.Position = UDim2.new(0,14,0,340+(i*15))
...
verText.Text = "⚡  NightHubX v4 • SmartAuto + DupFilter"
verText.Font = Enum.Font.GothamMedium; verText.TextSize = 10
...
-- Karakter tetangga QWERTY untuk typo realistis
...
local function GetTypoChar(ch)
	local n = QWERTY_NEIGHBORS[ch]
	if n and #n > 0 then return n[math.random(1,#n)] end
	return ch == "a" and "s" or "a"
end

local function TypeWord(rem_word, turnStartTime)
	local rem = TYPING_CONFIG.hardDeadline - (tick() - turnStartTime)
	if rem <= 1.0 then BillboardUpdate:FireServer(rem_word); return true end

	-- Pakai CustomCharDelay sebagai delay per karakter
	local charD = CustomCharDelay
	-- Speed multiplier: pastikan semua karakter selesai sebelum deadline
	local typoBuffer = TypoEnabled and (#rem_word * 0.18 * 0.45) or 0
	local est = #rem_word * charD + TYPING_CONFIG.submitDelayMax + typoBuffer
	local sp = 1.0
	if est > rem - 0.5 then sp = math.max((rem - 0.5) / est, 0.3) end

	local typed = ""
	for i = 1, #rem_word do
		if not AutoAnswerEnabled then return false end
		if TYPING_CONFIG.hardDeadline - (tick() - turnStartTime) <= 0.8 then
			BillboardUpdate:FireServer(rem_word); return true
		end

		local ch = string.sub(rem_word, i, i)

		-- Typo: ketik karakter salah → pause sebentar → hapus → ketik benar
		if TypoEnabled and math.random() < 0.18 and i < #rem_word then
			local wrong = GetTypoChar(ch)
			-- Kirim karakter salah
			typed = typed .. wrong
			BillboardUpdate:FireServer(typed)
			task.wait(math.random(12, 40) / 100 * sp)  -- 0.12 – 0.40s kelihatan salah
			if not AutoAnswerEnabled then return false end
			-- Hapus (backspace)
		...
		-- Delay per karakter dari CustomCharDelay
		local d = charD * sp
		-- Sesekali jeda natural
		if math.random() < TYPING_CONFIG.pauseChance and i < #rem_word then
			d = d + (TYPING_CONFIG.pauseDelayMin + math.random()*(TYPING_CONFIG.pauseDelayMax - TYPING_CONFIG.pauseDelayMin)) * sp
		end
		task.wait(d)
	end
	return true
...
local function DoAutoAnswer(turnStartTime)
	...
	if not Options or #Options == 0 then
		...
		SendNotification("warning", "NightHubX", "Tidak ada kata untuk '" .. CurrentLetter:upper() .. "'", 3)
		return
	end

	local word = Options[1]
	MarkWordAsUsed(word, "auto_answer")
	LastSubmittedWord = word

	if autoAnswerStatusLabel then
		local typoTag = TypoEnabled and " ⚡typo" or ""
		autoAnswerStatusLabel.Text = '⏳ "'..word..'" 🤖 '..string.format("%.2f",CustomCharDelay)..'s'..typoTag
		autoAnswerStatusLabel.TextColor3 = Color3.fromRGB(140, 100, 255)
	end

	-- Think delay
	local thinkD = TYPING_CONFIG.thinkDelayMin + math.random()*(TYPING_CONFIG.thinkDelayMax - TYPING_CONFIG.thinkDelayMin)
	...
	local rem_word = string.sub(word, #CurrentLetter + 1)
	local ok = TypeWord(rem_word, turnStartTime)
	...
	SubmitWord:FireServer(rem_word)

	local lc = string.sub(word, -1)
	local _, dText = GetDangerCategory(word)
	local t = string.format("%.1f", tick() - turnStartTime)
	if autoAnswerStatusLabel then
		autoAnswerStatusLabel.Text = '✅ "'..word..'" '..dText..' ('..t..'s)'
	end
	SendNotification("auto", "Auto 🤖", '"'..word..'" → '..lc:upper().." "..dText.." ["..t.."s]", 2.5)
	UpdatePreview()
end
...
print("[NightHubX v4] SmartAuto+DupFilter initialized!")
print("[NightHubX v4] Tema: Night/Crimson/Cyan/Gold/Void | Strategy+ Engine | 13s deadline")
